...
void FurryPrimitive::setRangeOrientation(int _r){
	rangeorien = _r;
}

void FurryPrimitive::setRangeScale(int _r){
	rangescale = _r;
}

void FurryPrimitive::setScale(ofVec3f _s){
	scale = _s;
}

void FurryPrimitive::setOrientation(ofVec3f _d){
	direction = _d;
}

void FurryPrimitive::setTexture(ofImage _i){
	ofDisableArbTex();
	texture = _i;
}

void FurryPrimitive::setNoise(float _t, bool _n){
	noise = _n;
	valueNoise = _t;
}

void FurryPrimitive::setCustomMesh(ofMesh _m){
	mesh = _m;
	mesh.setUsage(GL_STATIC_DRAW);
	CustomMesh = true;
}

void FurryPrimitive::setPrimitive(int prim){
	if(prim == 0)
	{
...
void FurryPrimitive::setup(ofMesh _m){
	resizedMesh = 25;
	noise = false;
...
int FurryPrimitive::getNumVert()
{
	return xmesh.getNumVertices();
}

void FurryPrimitive::setVecDirection(vector<ofVec3f> _d)
{
	_dir = _d;
}

void FurryPrimitive::update(ofMesh _m){
	xmesh = _m;
	...
	for(size_t i=0;i<xmesh.getNumVertices();i++){
		...
		{
			rot.set(
				xmesh.getVertex(i).x*_dir[i].x,
				xmesh.getVertex(i).y*_dir[i].y,
				xmesh.getVertex(i).z*_dir[i].z
			);
		}
		else{
			rot.set(
...
ofMesh FurryPrimitive::getMeshResized(){
	ofMesh tmp = xmesh;	
...
void FurryPrimitive::draw(ofPolyRenderMode type){
	shader.begin();
...
ofMesh FurryPrimitive::ribbon(vector<ofVec3f> points, float _thickness){
    ofVboMesh mesh;
